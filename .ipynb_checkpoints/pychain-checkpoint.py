...
class PyChain:
    ...
    def create_and_add_record(self, sender, receiver, amount):
        # Create a new Record instance
        new_record = Record(sender, receiver, amount)
        
        # I created a new Block with the Record and add it to the blockchain
        new_block = Block(new_record, len(self.chain))
...
# Example of adding a record to the blockchain
if st.button("Add Record"):
    sender = "Alice"
    receiver = "Bob"
    amount = 10.0
    pychain.create_and_add_record(sender, receiver, amount)
...
sender = st.text_input("Enter Sender's Name")
st.write(f"Sender: {sender}")


# @TODO:
# Add an input area where you can get a value for `receiver` from the user.
receiver = st.text_input("Enter Receiver's Name")
